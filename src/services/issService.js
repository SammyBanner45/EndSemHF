import { normalizeLongitude } from '../utils/geo'

const ISS_NOW_URL = 'https://api.wheretheiss.at/v1/satellites/25544'
const ISS_FALLBACK_URL = 'http://api.open-notify.org/iss-now.json'

function toPosition(latitude, longitude, timestamp) {
  return {
    latitude: Number(latitude),
    longitude: normalizeLongitude(longitude),
    timestamp: Number(timestamp) * 1000,
  }
}

export async function fetchIssPosition() {
  let primaryError

  try {
    const response = await fetch(ISS_NOW_URL)
    if (!response.ok) throw new Error(`ISS feed returned ${response.status}`)

    const data = await response.json()
    return toPosition(data.latitude, data.longitude, data.timestamp)
  } catch (error) {
    primaryError = error
  }

  try {
    const response = await fetch(ISS_FALLBACK_URL)
    if (!response.ok) throw new Error(`ISS fallback returned ${response.status}`)

    const data = await response.json()
    if (data.message !== 'success') throw new Error('ISS feed did not return a valid position')

    return toPosition(data.iss_position.latitude, data.iss_position.longitude, data.timestamp)
  } catch (fallbackError) {
    const message = fallbackError.message || primaryError?.message || 'Please retry.'
    throw new Error(`Unable to load ISS position. ${message}`, { cause: fallbackError })
  }
}
